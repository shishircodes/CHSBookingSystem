      ...
      <div className="overflow-x-auto card p-0">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {bookings.map((b) => (
              <tr key={b.id}>
                <td className="px-4 py-3 whitespace-nowrap">{new Date(b.appointment_at).toLocaleString()}</td>
                ...
                <td className="px-4 py-3 space-x-1">
                  {user?.role === 'patient' && b.status !== 'cancelled' && b.status !== 'completed' && (
                    <button onClick={() => setStatus(b.id, 'cancelled')} className="btn-ghost text-xs py-1 px-2">Cancel</button>
                  )}
                  {user?.role === 'provider' && (
                    <>
                      {b.status === 'pending'   && <button onClick={() => setStatus(b.id, 'confirmed')} className="btn-primary text-xs py-1 px-2">Confirm</button>}
                      {b.status === 'confirmed' && <button onClick={() => setStatus(b.id, 'completed')} className="btn-primary text-xs py-1 px-2">Complete</button>}
                  ...
                  {user?.role === 'admin' && (
                    <>
                      ...
                      <button onClick={() => remove(b.id)} className="btn-danger text-xs py-1 px-2">Delete</button>
                    </>
                  )}
                </td>
              </tr>
            ...
            {bookings.length === 0 && (
              <tr><td colSpan="6" className="px-4 py-10 text-center text-slate-500">No bookings yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
